...
function sufficientstatistics(::Union{<:KnownExponentialFamilyDistribution{Erlang}, <:Erlang}, x) 
    @assert 0 <= x "Erlang sufficientstatistics should be evaluated at positive values"
    return [log(x), x]
end