...
struct MvNormalWishart{T, M<:AbstractArray{T}, V<:AbstractMatrix{T}, K<:Real, N<:Real} <: ContinuousMatrixDistribution
    μ::M
    Ψ::V
    κ::K
    ν::N

    function MvNormalWishart(μ::M, Ψ::V, κ::K, ν::N) where {T, M<:AbstractArray{T}, V<:AbstractMatrix{T}, K<:Real, N<:Real}
...
sufficientstatistics(::Union{<:KnownExponentialFamilyDistribution{MvNormalWishart}, <:MvNormalWishart}) = (x,Λ) -> [Λ*x, Λ, Λ*x*x', chollogdet(Λ)]
...
basemeasure(d::Union{<:KnownExponentialFamilyDistribution{MvNormalWishart}, <:MvNormalWishart}, x) = 1/(2*pi)^(dim(d)/2)


function Random.rand(rng::AbstractRNG, dist::MvNormalWishart{T}) where {T}
    μ,Ψ,κ,ν =  params(dist)
    wishartsample  = rand(rng, Wishart(ν, Ψ))
    normalsample   = rand(rng, MvNormalMeanPrecision(μ,κ*wishartsample))
    return [normalsample, wishartsample]
end

function Random.rand(rng::AbstractRNG, dist::MvNormalWishart{T}, size::Int64) where {T}
    container = Array{UnioArray{T}}(undef, 2, size)
    return rand!(rng, dist, container)
end

function Random.rand!(rng::AbstractRNG, dist::MvNormalWishart, container::AbstractArray{T}) where {T <: Real}
